...
export class Util {
  static cmd(base: string, cmd: string | string[], opts?: ExecOptions): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(`${base} ${cmd}`, opts, (error, stdout, stderr) => {
        ...
        if (stderrStr.length > 0) {
          // Git sends progress to stderr, so we only reject if it looks like an error?
          // The old code rejected if stderr > 0. Let's keep it for now but be wary.
          reject(stderrStr.trim());
          return;
        }
  ...
  static async multiCommand(args: string[], executor: (args: string[], file: FileEntry) => Promise<string>): Promise<void> {
  ...
  static renderTemplate<T>(template: (getNumber: (name: string, data: T) => number, data: any) => string, type: string, data: any): string {
    const map: EntryMap = {};
    let nextNumber = 1;
